...
def file_refactor(Mcalc=None):
    ...
    refactors the given user data into actionable permittivity and permeability data.
    ...
    if Mcalc is None:
        ErrorMsg = 'Data must be passed as an array which is mappable ' \
                   'to an Nx5 numpy array with columns [freq, e1, e2, mu1, mu2]'
...
def f_set_ref(f_set, Mcalc):
    ...
    :param Mcalc:   uses Mcalc as reference as frequencies are
